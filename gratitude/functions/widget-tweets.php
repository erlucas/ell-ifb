	...
	function widget( $args, $instance ) {
		...
		if ( 'error' != $tweets ) :
			...
			if ( isset( $instance['beforetweet'] ) && !empty( $instance['beforetweet'] ) )
				$before_tweet = stripslashes( wp_filter_post_zenes( $instance['beforetweet'] ) );
			...
			foreach ( (array) $tweets as $tweet ) {
				...
				/*
				 * Create linzen from plain text based on Twitter patterns
			...
			if ( 401 == get_transient( 'widget-twitter-response-code-' . $this->number ) )
				echo '<p>' . wp_zenes( sprintf( __( 'Error: Please make sure the Twitter account is <a href="%s">public</a>.', 'framework' ), 'http://support.twitter.com/forums/10711/entries/14016' ), array( 'a' => array( 'href' => true ) ) ) . '</p>';
			else
...
add_action( 'widgets_init', 'zen_twitter_widget_init' );
function zen_twitter_widget_init() {
